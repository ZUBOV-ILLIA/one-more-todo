...
export default function Todo({ todo }: Props) {
  ...
  return (
    <div
      className={`mb-0.5 p-3 flex bg-neutral-800 ${todo.completed ? 'text-slate-500' : 'text-slate-100'} rounded justify-between relative`}
    >
      <div className={`flex grow ${todo.completed ? 'line-through' : ''}`}>
        <div
          className={`absolute left-1.5 bg-neutral-800 duration-300 ${!isRequest ? 'opacity-0 invisible' : ''}`}
        >
