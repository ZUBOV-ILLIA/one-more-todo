...
export default function CheckAllBtn() {
  ...
  function setAllCompleted() {
    ...
    axios
      .patch(`${API}/todos`, {
        ids: todoIds,
        action: 'toggle',
        completed: !completedEvery,
      })
      .then(res => {
        setCompletedEvery(!completedEvery);
        dispatch(
          setTodos(todos.map(el => ({ ...el, completed: !completedEvery }))),
        );
        setIsRequest(false);
      })
      .catch(err => {
  ...
  return (
    <div className="w-9 h-9 p-1 mb-4 relative" onClick={setAllCompleted}>
      <div
        className={`absolute h-7 w-7 top-1 left-1 ${!isRequest ? 'opacity-0 invisible' : ''} bg-black duration-300`}
      >
        <IconLoader />
      </div>
      <IconSuccess
        className={`${completedEvery ? 'fill-slate-100' : 'fill-slate-500'}`}
      />
    </div>
  );
}
